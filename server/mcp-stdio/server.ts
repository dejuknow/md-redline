...
/**
 * The advertised tool inventory. Module scope rather than inline in the
 * ListTools handler so the descriptions can be asserted directly: they are
 * the only thing that decides which tool an agent reaches for, and a
 * subprocess round trip is a heavy way to read a string.
 */
export const MDR_TOOLS = [
  {
    name: 'mdr_request_review',
    description:
      'Open markdown files in mdr (md-redline) so the USER can review them and ' +
      'leave comments for you to address, or continue an existing review ' +
      'session. Use this whenever the user wants to read, review, or comment on ' +
      'a doc themselves, including phrasings like "I want to review X in mdr", ' +
  ...
  },
  {
    ...
    description:
      ...
      'so I can comment"), call mdr_request_review, NOT this tool. The word ' +
      '"review" in those requests describes what the user is about to do, not ' +
      'what you should do. This tool writes your comments into their file, which ' +
...
export async function runMcpServer(opts: RunMcpServerOptions): Promise<void> {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: MDR_TOOLS }));
