...
export function insertComment(
  ...
  // Insert BEFORE the newline preceding the fence so the existing \n separates the marker
  // from the ``` fence (keeping it at line start). This also preserves round-trip integrity:
  // stripping the marker restores the original content exactly.
  let codeBlockNewlineNeeded = false;
  {
    const fenceRegex = /^(`{3,}|~{3,}).*$/gm;
    ...
    while ((fm = fenceRegex.exec(cleanMarkdown)) !== null) {
      ...
      if (!openF) {
        ...
        if (insertionCleanOffset >= openF.start && insertionCleanOffset <= fm.index + fm[0].length) {
          if (openF.start > 0 && cleanMarkdown[openF.start - 1] === '\n') {
            // Insert before the preceding newline; it naturally separates marker from fence
            insertionCleanOffset = openF.start - 1;
          } else {
            // Fence at document start — must add a newline after marker
            insertionCleanOffset = openF.start;
            codeBlockNewlineNeeded = true;
          }
        }
        openF = null;
      }
  ...
  const suffix = codeBlockNewlineNeeded ? '\n' : '';
  return rawMarkdown.slice(0, rawInsertionPoint) + marker + suffix + rawMarkdown.slice(rawInsertionPoint);
}
